...
class AutoModelServiceTest {
	...
	@MockBean
	private AutoModelMapper autoModelOrikaMapper;
    ...
    public void setUp() {
        autoModelService =
                new AutoModelServiceImpl(repository, autoModelOrikaMapper);
    }
...
//        Mockito.when(repository.save(argThat(new AutoModelMatcher(new AutoModel("briefName2"))))).thenReturn(getMockAutoModelEntity2());
        ...
        Mockito.when(autoModelOrikaMapper.map(any(AutoModel.class), any())).thenReturn(getAutoModelDtoEntity());
        Mockito.when(autoModelOrikaMapper.map(any(AutoModelDto.class), any())).thenReturn(getMockAutoModelEntity());
    }
    ...
    void create() {
    	...
    	System.out.println("autoModelMapper"+autoModelOrikaMapper);
